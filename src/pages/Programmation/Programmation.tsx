...
export default function Programmation(props : Props) {
  ...
  useEffect(() => {
    if (!location.state || oldGroupes == undefined){
      axios.get('https://www.api.festiuto.sebastien-gratade.fr:8080/getArtistes').then((res) => {
  ...
  useEffect(() => {
    ...
    if (filtreDate !== "Tout" || filtreGenre !== "Tout"){
      const requete = "https://www.api.festiuto.sebastien-gratade.fr:8080/getArtistes" + (filtreDate !== "Tout" ? "?date="+filtreDate : "") + (filtreDate !== "Tout" ? "&" : "?") + (filtreGenre !== "Tout" ? "genre="+filtreGenre : "");
      console.log(requete);
  ...
  const handleSearch = (searchText: string) => {
    ...
    const requete = "https://www.api.festiuto.sebastien-gratade.fr:8080/searchUsers?recherche="+searchText + (filtreDate !== "Tout" ? "&date="+filtreDate : "") + (filtreGenre !== "Tout" ? "&genre="+filtreGenre : "");
    axios.get(requete).then((res) => {
      if (res.status === 200){
        const data = res.data as Groupe[];
        setLesGroupes(data);
