...
export default function MenuConnexion(props: Props) {
  ...
  const handleResetArtists = () => {
    axios.get("http://51.178.46.205:8080/getArtistesWithSave?idUser="+getUserCookie().idUser+ (filtreDate !== "Tout" ? "&date="+filtreDate : "") + (filtreGenre !== "Tout" ? "&genre="+filtreGenre : "")
    ).then(((res) => {
          if (res.status === 200){
            const data = res.data;
  ...
  const handleSearch = (searchText : string) => {
    ...
    const requete = "http://51.178.46.205:8080/searchUsersWithSave?recherche="+searchText+"&idUser="+getUserCookie().idUser + (filtreDate !== "Tout" ? "&date="+filtreDate : "") + (filtreGenre !== "Tout" ? "&genre="+filtreGenre : "");
  ...
  useEffect(() => {
    ...
    if (currentMenu === "planification"){
      if (filtreDate !== "Tout" || filtreGenre !== "Tout"){
        const requete = "http://51.178.46.205:8080/getArtistesWithSave?idUser=" + getUserCookie().idUser + (filtreDate !== "Tout" ? "&date="+filtreDate : "") + (filtreGenre !== "Tout" ? "&genre="+filtreGenre : "");
        console.log(requete);
        axios.get(requete).then((res) => {
          if (res.status === 200){
            const data = res.data as Groupe[];
  ...
  useEffect(() => {   
    if (props.isOpen){
      ...
      if (currentMenu === "connecte"){
        axios.get("http://51.178.46.205:8080/getMesBillets?idUser="+getUserCookie().idUser).then((res) => {
          if (res.status === 200){
      ...
      if (currentMenu === "affichage-planification"){
        axios.get("http://51.178.46.205:8080/getMonPlanning?idUser="+getUserCookie().idUser).then((res) => {
          if (res.status === 200){
            const data = res.data;
            for (let key in data){
              console.log(key);
  ...
  const handleDownload = () => {
    ...
    axios.post('http://51.178.46.205:8080/telecharger_planif',{
    responseType: 'blob',
  ...
  const handleConnexion = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/connecter", data).then((res) => {
      const data = res.data;
      if (data.error){
  ...
  const handleInscription = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/inscription", data).then((res) => {
      const data = res.data;
  ...
  const handleResetMdp = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/envoyerCodeVerification", data).then((res) => {
  ...
  const handleEnvoyerCode = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/testerCodeVerification", data).then((res) =>{
      const dataRes = res.data;
      if(dataRes.error){
  ...
  const handleModifierInfos = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/modifierProfil", data).then((res) => {
      const data = res.data;
  ...
  const handleModifierMdp = (e : React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post("http://51.178.46.205:8080/modifierMdp", data).then((res) => {
      const dataRes = res.data;
