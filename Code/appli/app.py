...
def telecharger_billet():
    connexion_bd = ConnexionBD()
    billetbd = BilletBD(connexion_bd)
    """
    axios.post('http://localhost:8080/telecharger_billet', {
        responseType: 'blob',    
        data: props.billet
        })
    """
    data = request.get_json()
    print(data)
    idBillet = data["data"]["idB"]
    print(idBillet)
    billet = billetbd.get_billet_by_id(idBillet)
    print(billet.to_dict())
    ...
    p = Paragraph("Mes billets", style=styles["BodyText"])
    p.wrapOn(c, 200, 600)
    ...
    p = Paragraph("GRATADE SEBASTIEN", style=styles["BodyText"])
    p.wrapOn(c, 200, 600)
    p.drawOn(c, width-250, height-150)
    
    # en dessous de ces deux textes je veux mettre le billet en grand (qui prenne toute la largeur)
    
    #c.drawImage(img_billet_path, 59, height-413, width=484, height=167.3)
    
    # je veux dessiner un tableau horaire artiste genre scene
    
    tableau = [["Horaire", "Artiste", "Genre", "Scene"]]
    tableau.append(["14h00", "Muse", "Rock", "Scene 1"])
    tableau.append(["15h00", "Muse", "Rock", "Scene 1"])
    tableau.append(["16h00", "Muse", "Rock", "Scene 1"])
    tableau.append(["16h00", "Muse", "Rock", "Scene 1"])
    ...
    style.fontName="Bold"
    p = Paragraph("22 JUILLET", style=styles["BodyText"])
    p.wrapOn(c, 200, 600)
    p.drawOn(c, 59, height-200)
    ...
    t.wrapOn(c, 500, 600)
    t.drawOn(c, 59, height-(63*len(tableau)))
    
    # en dessous je veux écrire les conditions du billet:
...
<br/>9.Remerciements et encouragement : "Nous vous remercions d'être des participants essentiels à cette expérience. Profitez pleinement de FEST IUT’O et créez des souvenirs mémorables. Nous sommes ravis de partager ces moments exceptionnels avec vous."
    ...
    connexion_bd.fermer_connexion()
    return send_file("billet.pdf", as_attachment=True)