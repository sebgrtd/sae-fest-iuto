...
from gen_pdf.Generateur import *
...
def telecharger_billet():
    ...
    get_pdf_billet("GRATADE", "SEBASTIEN")
    
    connexion_bd.fermer_connexion()
    return send_file("billet.pdf", as_attachment=True)

@app.route("/telecharger_planif", methods=["POST"])
def telecharger_planif():
    connexion_bd = ConnexionBD()
    data = request.get_json()
    
    get_pdf_planing("GRATADE","SEBASTIEN",data['data'])
    
    connexion_bd.fermer_connexion()
    return send_file("planification.pdf", as_attachment=True)